...
const buttonVariants = cva(
  [
    "group relative inline-flex cursor-pointer items-center justify-center outline-none",
    ...
    return (
      ...
      >
        {loading ? (
          ...
          <span className="[&_svg]:stroke-[1.5] [&_svg]:transition-[stroke-width] [&_svg]:duration-100 group-hover:[&_svg]:stroke-[2]">
            {children}
          </span>
        ) : (
          <>
            {LeadingIcon && (
              <LeadingIcon
                size={iconSize}
                strokeWidth={1.5}
                className="transition-[stroke-width] duration-100 group-hover:stroke-[2]"
              />
            )}
            <span>{children}</span>
            {TrailingIcon && (
